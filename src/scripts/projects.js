...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (Array.isArray(projects) && projects.length > 0) renderProjects(projects);
  } catch (_) {}
});
        ...
        <div class="project-card-header">
          ...
          <div class="project-status-wrap">
            <span class="badge ${p.isFeatured ? 'badge-live' : 'badge-done'}">
              ${p.isFeatured ? '<span class="badge-dot"></span>Live' : 'Completed'}
            </span>
            <div class="project-links-row">${links}</div>
          </div>
        </div>
        <h3 class="project-title">${p.title}</h3>
        ${p.date ? `<p class="project-date">${p.date}</p>` : ''}
